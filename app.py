...
try:
    ...
    if not df_opcoes.empty and 'doenca' in df_opcoes.columns:
        ...
        # FILTRO: Só mostra o que ainda não foi chutado
        lista_disponivel = [d for d in lista_completa if d not in st.session_state.chutes_feitos]
    else:
        lista_disponivel = []
    ...
    if jogo_hoje.empty:
        ...
        if not st.session_state.terminou:
            palpite = st.selectbox("Selecione seu diagnóstico (opções restantes):", [""] + lista_disponivel)
            
            if st.button("Confirmar Hipótese"):
                if palpite == "":
                    st.warning("Selecione uma doença!")
                else:
                    # Adiciona o palpite à lista de "já usados"
                    ...
                    if normalizar(palpite) == normalizar(solucao):
                        ...
                        if st.session_state.tentativas >= 6:
                            ...
                            st.error(f"Incorreto! '{palpite}' foi descartado.")
                        st.rerun()
        ...
        if st.session_state.terminou:
            ...
            if st.button("Resetar jogo (Testes)"):
